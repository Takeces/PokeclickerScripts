// ==UserScript==
// @name         PokeClicker Auto Safari PKRS
// @namespace    http://tampermonkey.net/
// @version      0.3
// @description  Auto Safari pokerus catching
// @author       Takeces
...
(function() {
	...
	var autoTimeout = 250;
    const BUTTON_ID = 'pcDoAutoSafariPkrs';
    ...
    function init() {
        ...
        btn.innerHTML = 'Auto Safari PKRS';
    ...
    function needToCatch(pkmnId) {
        const status = PartyController.getPokerusStatus(pkmnId);
        return status == GameConstants.Pokerus.Infected || status == GameConstants.Pokerus.Contagious;
    }
...
/*                     if (Safari.completed(true)) {
                        ...
                        if (
                            Safari.grid[y][x] == 10
                            && matrix[y][x] < nearestGrass.d
                            && next.map(({x,y}) => Safari.grid[y][x]).includes(10)
                        ) {
                            nearestGrass = {x, y, d: matrix[y][x]};
                        }
/*                     } */
        ...
        if (Safari.inProgress() && document.querySelector('#safariModal').classList.contains('show')) {
            if (Safari.inBattle()) {
                if (!SafariBattle.busy()) {
                    if(needToCatch(SafariBattle.enemy.id)) {
