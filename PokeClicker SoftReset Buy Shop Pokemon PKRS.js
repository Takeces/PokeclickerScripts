// ==UserScript==
// @name         PokeClicker SoftReset Buy Shop Pokemon PKRS
// @namespace    pcInfoStuff
// @version      0.1
// @description  Soft reset for buying shop pokemon for PKRS
// @author       Takeces
...
(function() {
    ...
    function checkGameLoadedAndRunning() {
        ...
        (function() {
            var ogFunc = App.game.party.gainPokemonById; // <-- Reference
            App.game.party.gainPokemonById = function(id, shiny = false, suppressNewCatchNotification = false, gender = PokemonFactory.generateGenderById(id), shadow = GameConstants.ShadowStatus.None) {
                ogFunc.apply(this, [id, shiny, suppressNewCatchNotification, gender, shadow]);
                boughtShiny = shiny;
            };
        })();
    ...
    function doTheThing() {
        ...
        const items = shops.map(shop => shop.items.filter(i => i instanceof PokemonItem && i.getPokerusStatus() != GameConstants.Pokerus.Resistant));
...
/*             let deals = GenericDeal.list[shops[i].name](); */
            let deals = GenericDeal.list[shops[i].traderID]();
            for(let j = 0; j < deals.length; j++) {
                let profits = deals[j].profits;
                for(let k = 0; k < profits.length; k++) {
                    if(profits[k].item instanceof PokemonItem && profits[k].item.getPokerusStatus() != GameConstants.Pokerus.Resistant) {
                        // reset boughtShiny
                        boughtShiny = false;
                        // buy item
        ...
        for(let i = 0; i < shops.length; i++) {
            if(!(shops[i] instanceof ShardTraderShop)) continue;
            let deals = ShardDeal.list[shops[i].location]();
            for(let j = 0; j < deals.length; j++) {
                let item = deals[j].item;
                if(item.itemType instanceof PokemonItem && item.itemType.getPokerusStatus() != GameConstants.Pokerus.Resistant) {
                    // reset boughtShiny
                    boughtShiny = false;
                    // buy item
                    ShardDeal.use(shops[i].location, j, 1);
        ...
        for(let i = 0; i < shops.length; i++) {
            ...
            for(let j = 0; j < deals.length; j++) {
                if(deals[j].item.itemType instanceof PokemonItem && deals[j].item.itemType.getPokerusStatus() != GameConstants.Pokerus.Resistant) {
                    // reset boughtShiny
                    boughtShiny = false;
                    // buy item
